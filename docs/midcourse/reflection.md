...
A second real lesson came after submission: the instructor found two live bugs
(a same-status PATCH always failing, and a stray Unicode character risking a
JS syntax error) that my own manual testing had missed. Both were fixed by
reproducing the exact reported behavior first — hitting the API with the same
request shape the frontend actually sends, and driving the real edit flow through
a headless browser — rather than guessing at a fix from the bug description alone.
...
rejection, and the two post-submission bugs) only became trustworthy once I forced
it through tests, reproduction steps, and a real break-and-restore cycle instead of
taking the first green run at face value.
